...
* **Continuous metrics**: a Streamlit multiselect, plus a 'preview' row that
  shows a tiny histogram for the first three picked columns with a one-line
  summary beneath each chart.
...
Phase 4: voice-aware labels and histogram summaries.
"""
...
def render_metric_picker(
    ...
    *,
    ...
    metric_cols = st.multiselect(
        copy.step3_outcome_label(voice),
        numeric_cols,
    ...
    for slot, col in zip(cols_layout, cols):
        try:
            ...
            with slot:
                ...
                if summary:
                    st.markdown(
                        f"<div style='color:var(--tl-slate);font-size:.78rem;"
                        f"line-height:1.4;margin-top:-8px;'>{summary}</div>",
...
def _render_ratio_rows(df: pl.DataFrame, default_ratios: List[dict]) -> List[RatioMetric]:
    ...
    for i in range(int(n_rows)):
        ...
        st.markdown(
            f"<div class='tl-label-heavy' style='margin-top:.75rem;'>"
            f"Ratio {i + 1}</div>",
            unsafe_allow_html=True,
        )
